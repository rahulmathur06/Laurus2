...
class ReporteDisponibilidadController extends Controller {
	...
	public function getIndex()
	{
        ...
        dd('USER EMAIL = '.$user->email.'', 'USER ID = '.$user->id.'');*/

        return view('flota::reportes1.index');
	}
        ...
        if(Sentinel::check()){
            if(Sentinel::hasAccess('disponibilidad.view')){
                ...
                if ($operator_plaza = $request->get('operator_plaza')) {
                    $datatables->where('o.plaza', 'like', "$operator_plaza%"); // additional plaza search
                }
