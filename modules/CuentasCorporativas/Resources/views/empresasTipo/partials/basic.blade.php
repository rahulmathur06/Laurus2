...
<!--    <legend>Captura de Siniestro</legend>-->
    ...
    <div class='row'>
        <div class='col-md-2'>
            @if(isset($EmpresasTipo))
            <div class="form-group @if ($errors->has('empresas_id')) has-error @endif">
                <label class="control-label">
                    <span  data-toggle="tooltip" data-html="true" title="" data-original-title="Nombre del automóvil. Caracteres invalidos ·$%&/()=?">
                        ID
                    </span>
                </label>
                {!! Form::number('id',null,['id'=>'empresas_id','class' => 'form-control', 'disabled' => true]) !!}
            </div><!-- /.form-group -->
            @endif
        </div>
